    ...
    public void invoke(Request request, Response response) throws IOException,
        ...
        for (int j = 0; j < allows.length; j++) {
            ...
            if (allows[j].matcher(remoteAdress).matches()) {
                ...
                //modification pour opencloudware
                String role="users";
                if (log.isDebugEnabled())
                ...
                if (user != null) {
                    ...
                    //in Tomcat 7 realm parameter is absent
                    Principal principal = new GenericPrincipal(user, "", roles);
                    request.setUserPrincipal(principal);
                    //request.getContext().getRealm().authenticate(principal.getName(),"");
                } else if (!passThrough) {
                    if (log.isDebugEnabled())
